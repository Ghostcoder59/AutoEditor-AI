...
import { extractFeaturesFromFile, inferInBrowserOrServer } from './featureExtractor';
...
const buildWaveformPlot = (samples) => {
  const safeSamples = Array.isArray(samples)
    ? samples.map((value) => Math.max(0, Math.min(1, Number(value) || 0)))
    : [];

  if (!safeSamples.length) {
    return { points: '', width: 0, height: 0, area: '', min: 0, max: 0 };
  }

  const width = Math.max(720, safeSamples.length * 10);
  const height = 156;
  const usableHeight = height - 30;
  const step = safeSamples.length > 1 ? width / (safeSamples.length - 1) : width;

  const points = safeSamples
    .map((sample, index) => {
      const x = Math.round(index * step);
      const y = Math.round(height - 18 - sample * usableHeight);
      return `${x},${y}`;
    })
    .join(' ');
...
const colorForSample = (v) => {
  const val = Math.max(0, Math.min(1, Number(v) || 0));
  if (val >= 0.75) return 'linear-gradient(180deg, #fb923c, #f97316)'; // High (orange)
  if (val >= 0.5) return 'linear-gradient(180deg, #67e8f9, #10b981)'; // Medium-high (teal)
  if (val >= 0.25) return 'linear-gradient(180deg, #a78bfa, #60a5fa)'; // Medium (indigo/blue)
  return 'linear-gradient(180deg, #c7d2fe, #bfdbfe)'; // Low (soft blue)
};
...
function App() {
  ...
  const [waveformMode, setWaveformMode] = useState(localStorage.getItem('autoeditor_waveform_mode') || 'plot');
  const [analysisProgress, setAnalysisProgress] = useState(null);
  const [urlInput, setUrlInput] = useState('');
  const [exportingYouTube, setExportingYouTube] = useState(false);
  const [useAi, setUseAi] = useState(true);
  const [browserInference, setBrowserInference] = useState(true);
  const [browserInferenceBusy, setBrowserInferenceBusy] = useState(false);
  const [browserInferenceResult, setBrowserInferenceResult] = useState(null);
  ...
  const scrollWaveform = (direction = 1) => {
  ...
  React.useEffect(() => {
    localStorage.setItem('autoeditor_waveform_mode', waveformMode);
  }, [waveformMode]);
  ...
  React.useEffect(() => {
    const onMessage = async (event) => {
      ...
      if (!event.data || event.data.type !== 'youtube-connected' || !token) return;
      ...
      try {
        ...
        console.error('Failed to refresh user after YouTube connect', error);
      }
    };
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (authMode === 'login' || authMode === 'register') {
        ...
        if (data.debug_reset_code) {
          setResetToken(data.debug_reset_code);
        }
        setAuthMode('reset');
        setAuthMessage('Check your Gmail for the verification code, then enter it below with a new password.');
      } else if (authMode === 'reset') {
  ...
  const handleFileSelect = async (e) => {
    ...
    setBrowserInferenceResult(null);
    setStatus('UPLOADING');
    setProgress(15);
    setMessage(browserInference ? 'Analyzing in your browser first...' : 'Uploading and validating your video...');

    if (browserInference) {
      setBrowserInferenceBusy(true);
      try {
        const features = await extractFeaturesFromFile(file, { segmentLength: 1.0 });
        const result = await inferInBrowserOrServer(features, {
          tfjsModelBase: '/models/cheer_tfjs',
          apiBase: API_BASE_URL,
          token,
          threshold: 0.6,
        });

        const cheerCount = Array.isArray(result?.preds) ? result.preds.filter(Boolean).length : 0;
        setBrowserInferenceResult({
          backend: result?.backend || 'unknown',
          probs: result?.probs || [],
          preds: result?.preds || [],
          cheerCount,
          segmentCount: Array.isArray(result?.preds) ? result.preds.length : 0,
        });
        setMessage(`Browser analysis complete (${result?.backend || 'tfjs'}).`);
        setProgress(30);
      } catch (err) {
        setBrowserInferenceResult({ backend: 'error', error: err.message || 'Browser inference failed' });
      } finally {
        setBrowserInferenceBusy(false);
      }
    }
  ...
  const checkStatus = async (jobId) => {
    try {
      ...
      if (data.status === 'success') {
        ...
        setAuthMessage('Processing complete. You can preview, download, or publish the result below.');
    ...
    <div className="auth-shell">
      ...
      {authMode === 'forgot' && (
        <p className="auth-helper">
          Enter your email or username, then check Gmail for the reset code and come back here to finish the reset.
        </p>
      )}

      {authMode === 'reset' && (
        <p className="auth-helper">
          Paste the 6-digit code from Gmail, set a new password, and continue back to login.
        </p>
      )}
    </div>
  );
          ...
          <div className="ai-suite-card" style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                      {Array.isArray(liveWaveform) && liveWaveform.length > 0 && (() => {
                        const waveformSamples = liveWaveform.map((value) => Math.max(0, Math.min(1, Number(value) || 0)));
                        const waveformPlot = buildWaveformPlot(waveformSamples);

                        return (
                          <div className="live-wave-card" style={{ marginTop: '1.25rem', width: 'min(920px, 100%)' }}>
                            <div className="live-wave-head">
                              <span>Audio timeline</span>
                              <span>{waveformSamples.length} samples</span>
                            </div>
                            <div className="wave-mode-toggle">
                              <button
                                type="button"
                                className={`wave-mode-btn ${waveformMode === 'bars' ? 'is-active' : ''}`}
                                onClick={() => setWaveformMode('bars')}
                              >
                                Bars
                              </button>
                              <button
                                type="button"
                                className={`wave-mode-btn ${waveformMode === 'plot' ? 'is-active' : ''}`}
                                onClick={() => setWaveformMode('plot')}
                              >
                                Plot
                              </button>
                            </div>
                            {waveformMode === 'bars' ? (
                              <>
                                <div className="live-wave-controls">
                                  <button type="button" className="wave-nav-btn" onClick={() => scrollWaveform(-1)} aria-label="Scroll waveform left">
                                    ←
                                  </button>
                                  <button type="button" className="wave-nav-btn" onClick={() => scrollWaveform(1)} aria-label="Scroll waveform right">
                                    →
                                  </button>
                                </div>
                                <div className="live-wave-scroll" ref={waveformScrollRef} aria-label="Scrollable waveform timeline">
                                  <div className="live-wave-bars" aria-label="Full waveform preview">
                                    {waveformSamples.map((sample, idx) => (
                                      <span
                                        key={`wave-success-${idx}`}
                                        className="live-wave-bar"
                                        style={{
                                          height: `${10 + sample * 46}px`,
                                          background: colorForSample(sample),
                                          width: 12,
                                          marginRight: 6,
                                          borderRadius: 6,
                                ...
                                <div className="live-wave-legend" style={{ display: 'flex', gap: 12, marginTop: 10, alignItems: 'center' }}>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <div style={{ width: 18, height: 12, borderRadius: 4, background: 'linear-gradient(180deg, #c7d2fe, #bfdbfe)', border: '1px solid rgba(255,255,255,0.06)' }} />
                                    <span style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1 }}>Low intensity</span>
                                  </div>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <div style={{ width: 18, height: 12, borderRadius: 4, background: 'linear-gradient(180deg, #a78bfa, #60a5fa)', border: '1px solid rgba(255,255,255,0.06)' }} />
                                    <span style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1 }}>Medium</span>
                                  </div>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <div style={{ width: 18, height: 12, borderRadius: 4, background: 'linear-gradient(180deg, #67e8f9, #10b981)', border: '1px solid rgba(255,255,255,0.06)' }} />
                                    <span style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1 }}>High</span>
                                  </div>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <div style={{ width: 18, height: 12, borderRadius: 4, background: 'linear-gradient(180deg, #fb923c, #f97316)', border: '1px solid rgba(255,255,255,0.06)' }} />
                                    <span style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1 }}>Peak</span>
                                  </div>
                                </div>
                              </>
                            ) : (
                              <div className="live-wave-scroll live-wave-plot-scroll" ref={waveformScrollRef} aria-label="Scrollable waveform plot">
                                <svg
                                  className="live-wave-plot"
                                  width={waveformPlot.width}
                                  height={waveformPlot.height}
                                  viewBox={`0 0 ${waveformPlot.width} ${waveformPlot.height}`}
                                  role="img"
                                  aria-label="Live waveform plot"
                                  preserveAspectRatio="none"
                                >
                                  <defs>
                                    <linearGradient id="waveFillSuccess" x1="0" x2="0" y1="0" y2="1">
                                      <stop offset="0%" stopColor="rgba(103, 232, 249, 0.65)" />
                                      <stop offset="100%" stopColor="rgba(16, 185, 129, 0.08)" />
                                    </linearGradient>
                                    <linearGradient id="waveStrokeSuccess" x1="0" x2="1" y1="0" y2="0">
                                      <stop offset="0%" stopColor="#67e8f9" />
                                      <stop offset="50%" stopColor="#22d3ee" />
                                      <stop offset="100%" stopColor="#10b981" />
                                    </linearGradient>
                                  </defs>
                                  <rect x="0" y="0" width={waveformPlot.width} height={waveformPlot.height} rx="14" fill="rgba(255,255,255,0.04)" />
                                  <path d={`M ${waveformPlot.area} Z`} fill="url(#waveFillSuccess)" opacity="0.9" />
                                  <path d={`M ${waveformPlot.points}`} fill="none" stroke="url(#waveStrokeSuccess)" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />
                                  {[0.25, 0.5, 0.75].map((line) => (
                                    <line
                                      key={`grid-success-${line}`}
                                      x1="0"
                                      y1={Math.round(waveformPlot.height - 18 - line * (waveformPlot.height - 30))}
                                      x2={waveformPlot.width}
                                      y2={Math.round(waveformPlot.height - 18 - line * (waveformPlot.height - 30))}
                            ...
                            <div className="live-wave-foot">
                              <span>{waveformMode === 'plot' ? 'Plotted view shows the waveform as a line graph.' : 'Bars view is optimized for quick intensity scanning.'}</span>
                              <span>The card stays available after processing finishes.</span>
                            </div>
                          </div>
                        );
                      })()}
                ...
                <button 
                  className={`btn mini ${browserInference ? '' : 'secondary'}`}
                  onClick={() => setBrowserInference(!browserInference)}
                  style={{ 
                    borderRadius: '12px',
                    padding: '0.5rem 1rem',
                    marginLeft: '0.5rem',
                    background: browserInference ? 'var(--primary)' : 'rgba(255,255,255,0.05)',
                    border: browserInference ? 'none' : '1px solid rgba(255,255,255,0.1)'
                  }}
                >
                  {browserInference ? '🧠 Browser On' : 'Browser Off'}
                </button>
              </div>
              ...
              {useAi && (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '14px' }}>
                    {browserInference && (
                      <div style={{ padding: '0.9rem', borderRadius: '12px', background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.18)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                            <span style={{ fontSize: '0.9rem', fontWeight: 600 }}>Browser inference</span>
                            <span style={{ fontSize: '0.78rem', color: '#93c5fd' }}>
                              {browserInferenceBusy ? 'Analyzing locally before upload...' : 'Runs TF.js in the browser and falls back to server if needed.'}
                            </span>
                          </div>
                          {browserInferenceResult?.backend && (
                            <span style={{ fontSize: '0.75rem', padding: '0.25rem 0.55rem', borderRadius: '999px', background: 'rgba(255,255,255,0.08)' }}>
                              {browserInferenceResult.backend}
                            </span>
                          )}
                        </div>
                        {browserInferenceResult && !browserInferenceResult.error && (
                          <div style={{ marginTop: '0.75rem', display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '0.75rem' }}>
                            <div className="insight-card" style={{ background: 'rgba(255,255,255,0.04)' }}>
                              <CheckCircle size={16} />
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontSize: '0.8rem', fontWeight: 500 }}>Segments</span>
                                <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{browserInferenceResult.segmentCount}</span>
                              </div>
                            </div>
                            <div className="insight-card" style={{ background: 'rgba(255,255,255,0.04)' }}>
                              <Check size={16} />
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontSize: '0.8rem', fontWeight: 500 }}>Cheer segments</span>
                                <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{browserInferenceResult.cheerCount}</span>
                              </div>
                            </div>
                            <div className="insight-card" style={{ background: 'rgba(255,255,255,0.04)' }}>
                              <AlertCircle size={16} />
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontSize: '0.8rem', fontWeight: 500 }}>Mode</span>
                                <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{browserInferenceResult.backend}</span>
                              </div>
                            </div>
                          </div>
                        )}
                        {browserInferenceResult?.error && (
                          <div style={{ marginTop: '0.75rem', color: '#fca5a5', fontSize: '0.8rem' }}>
                            {browserInferenceResult.error}
                          </div>
                        )}
                      </div>
        ...
        <div className="loading-wrapper" style={{ animation: 'fadeIn 0.5s ease' }}>
          ...
          {waveformSamples.length > 0 && (
            <div className="live-wave-card">
              ...
              {waveformMode === 'bars' ? (
                ...
                <div className="live-wave-scroll live-wave-plot-scroll" ref={waveformScrollRef} aria-label="Scrollable waveform plot">
                  ...
                  >
                    ...
                    <rect x="0" y="0" width={waveformPlot.width} height={waveformPlot.height} rx="14" fill="rgba(255,255,255,0.04)" />
                    <path d={`M ${waveformPlot.area} Z`} fill="url(#waveFill)" opacity="0.9" />
                    <path d={`M ${waveformPlot.points}`} fill="none" stroke="url(#waveStroke)" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />
                    {[0.25, 0.5, 0.75].map((line) => (
                      <line
                        key={`grid-${line}`}
                        x1="0"
              ...
              <div className="live-wave-foot">
                <span>{waveformMode === 'plot' ? 'Plotted view shows the waveform as a line graph.' : 'Bars view is optimized for quick intensity scanning.'}</span>
                <span>Newest samples stay on the right.</span>
              </div>
            </div>
          )}
        </div>
