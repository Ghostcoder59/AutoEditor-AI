...
# Load the trained model with a fallback: try pickle, then joblib if available.
def _load_model(path):
    try:
        with open(path, "rb") as model_file:
            return pickle.load(model_file)
    except Exception as e_pickle:
        # If it's a git-lfs pointer file, the content will start with 'version https://git-lfs'.
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as tf:
                header = tf.read(128)
                if header.startswith("version https://git-lfs"):
                    print("Model file appears to be a Git LFS pointer. LFS objects are not present locally.")
        except Exception:
            pass
        ...
        if joblib is not None:
            try:
                ...
                print(f"Model load failed (pickle: {e_pickle}; joblib: {e_joblib}). Falling back to dummy model.")
        else:
            print(f"Model load failed with pickle ({e_pickle}). Falling back to dummy model.")

        # Provide a dummy model so the API can boot in degraded (audio-only) mode.
        class _DummyModel:
            def predict_proba(self, X):
                import numpy as _np
                n = getattr(X, 'shape', (len(X),))[0] if X is not None else 1
...
def cut_and_merge_clips(video_path, cheer_segments, output_dir, export_format="16:9", vision_engine=None, video_filter="none", smoothing_buffer_cls=None):
    ...
    try:
        ...
        temp_audiofile = os.path.join(output_dir, "highlight_reel_temp_audio.m4a")
...
def process_video(
    ...
    return {
        ...
        # Return the full waveform so the UI can render an interactive scrollable view.
        "live_audio_waveform": waveform_points,
        "analysis_progress_pct": 100,
    }
