...
_AUTH_DB_PATH = os.getenv("AUTH_DB_PATH", "").strip()
if _AUTH_DB_PATH:
    # Resolve relative DB paths from this file's directory so startup CWD does not change DB selection.
    DB_PATH = _AUTH_DB_PATH if os.path.isabs(_AUTH_DB_PATH) else os.path.join(os.path.dirname(__file__), _AUTH_DB_PATH)
else:
...
def create_password_reset_token(user_id: int, expires_minutes: int = 30) -> str:
    # Use a short numeric code so the user can type it from Gmail.
