...
def _send_password_reset_email(to_email: str, username: str, token: str) -> bool:
    if not (SMTP_HOST and SMTP_USER and SMTP_PASS and SMTP_FROM):
        logger.warning("SMTP is not configured; cannot send reset email for %s. Here is the reset code: %s", to_email, token)
        logger.warning("Reset Code: %s", token)
        return False

    msg = EmailMessage()
    msg["Subject"] = "AutoEditor Pro password reset code"
    msg["From"] = SMTP_FROM
    msg["To"] = to_email
    msg.set_content(
        (
...
def bg_process_url(job_id: str, url: str, output_folder: str, output_dir: str, user_id: int, use_ai: bool = True, export_format: str = "16:9", video_filter: str = "none"):
    ...
    def _on_progress(update: dict):
        ...
        now = time.time()
        if now - last_persist_at >= 1.5 or update.get("analysis_progress_pct") == 100:
    ...
    try:
        ...
        live_audio_waveform = process_result.get("live_audio_waveform") if process_result else []
        analysis_progress_pct = process_result.get("analysis_progress_pct") if process_result else None
        ...
        if not output_video_path or not os.path.exists(output_video_path):
            jobs[job_id].update({
                ...
                "live_audio_waveform": live_audio_waveform,
                "analysis_progress_pct": analysis_progress_pct,
                "download_url": None,
...
async def forgot_password(payload: ForgotPasswordRequest):
    ...
    if RESET_TOKEN_DEBUG:
        response["debug_reset_code"] = token
...
async def _process_youtube_callback(code: str, state: str, redirect_uri: str):
...
async def youtube_callback(code: str, state: str):
    return await _process_youtube_callback(code, state, YOUTUBE_BACKEND_REDIRECT_URI)


@app.get("/callback")
async def youtube_callback_legacy(code: str, state: str):
    return await _process_youtube_callback(code, state, "http://localhost:8000/callback")
...
async def infer_features(payload: InferFeaturesRequest, authorization: str | None = Header(default=None)):
    """Accepts pre-computed features and returns per-segment cheer probabilities and predictions.

    Payload: { features: [[f1,f2,...], ...], threshold: 0.6 }
    """
    # Require auth for access to inference (keeps usage consistent with other endpoints)
    user = None
    try:
        user = _require_user(authorization)
    except HTTPException:
        # If you prefer unauthenticated access, comment the above and allow anonymous usage.
        raise

    try:
        arr = np.array(payload.features, dtype=float)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid features: must be a list of numeric feature vectors")

    try:
        # Use the same model loaded by process_video (fallbacks to dummy if missing)
        probs = process_video.model.predict_proba(arr)[:, 1].tolist()
    except Exception as exc:
        logger.exception("Inference failed: %s", exc)
        raise HTTPException(status_code=500, detail="Model inference failed")

    preds = [1 if p >= float(payload.threshold) else 0 for p in probs]
    return {"probs": probs, "preds": preds, "threshold": payload.threshold}
